...
Precomputed crime forecasts generated by Prophet. Refreshed daily by Cron job. Feeds the Trend Analysis forecast overlay.

| Column Name | Type | Description |
|---|---|---|
| `ROWID` | INT PK | Unique identifier |
| `crime_category` | VARCHAR | Crime category (e.g., "Robbery", "Theft", "Assault") |
| `forecast_date` | DATE | Date for which this forecast applies |
| `predicted_value` | INT | Predicted crime count for this date |
| `lower_bound` | INT | Lower bound of 80% confidence interval |
| `upper_bound` | INT | Upper bound of 80% confidence interval |
| `generated_at` | DATETIME | When this forecast was generated |
| `forecast_days` | INT | Number of days forecasted (typically 30) |

**Key notes:**
- Forecasts are generated by Prophet models trained on historical data
- Each crime category has 30 forecast points (one per day)
- Old forecasts are deleted and replaced when new forecasts are generated
- Forecast data is visually distinct from historical data in the UI (dashed line, different color)
